...
public class StaticList<E> implements List<E> {
	...
	public int contaElementos(E el) throws IllegalArgumentException {
		int posicao = search(el);
		if(posicao == -1) throw new IllegalArgumentException();
		return contaElementosR(el, 0, 0);
	}

	private int contaElementosR(E el, int posicao, int quantidade) {
		if(posicao == numElements) return quantidade;
		else if(elements[posicao] == el) return contaElementosR(el, posicao + 1, quantidade + 1);
		else return contaElementosR(el, posicao + 1, quantidade);
	}
